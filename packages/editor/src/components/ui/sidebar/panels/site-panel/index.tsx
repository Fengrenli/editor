    ...
    <div className="relative border-border/50 border-b">
      ...
      <div className="relative flex items-center justify-between py-2 pr-3 pl-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-sm">{t('propertyLine')}</span>
        </div>
        <button
          className={cn(
  ...
  return (
    ...
    >
      ...
      <div className="flex h-8 min-w-0 flex-1 cursor-pointer items-center gap-2 py-0 pl-[60px] text-muted-foreground group-hover/ref:text-foreground">
        ...
        <InlineRenameInput
          defaultName={refNode.type === 'scan' ? t('defaultNameScan') : t('defaultNameGuide')}
          isEditing={isEditing}
          node={refNode}
...
function LevelReferences({
  ...
  const handleAddAsset = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!projectId) {
      ...
      useUploadStore.getState().setError(levelId, t('errorNoProject'))
      return
    }
    ...
    if (file.size > MAX_FILE_SIZE) {
      ...
      useUploadStore
        ...
        .setError(
          levelId,
          t('errorFileTooLarge', { sizeMb: (file.size / 1024 / 1024).toFixed(0) }),
        )
      return
    }
    ...
    if (!(isScan || isImage)) {
      ...
      useUploadStore
        .getState()
        .setError(levelId, t('errorInvalidFile'))
      return
    }
  ...
  const handleDelete = async (nodeId: string, e: React.MouseEvent) => {
    ...
    const url = refNode?.url
    if (url && onDeleteAsset) {
      const isRemote = url.startsWith('http://') || url.startsWith('https://')
      const isLocalAsset = url.startsWith('asset://')
      if (isLocalAsset || (isRemote && projectId)) {
        onDeleteAsset(projectId ?? 'local', url)
      }
    }
    ...
    <div className="relative flex flex-col">
      ...
      >
        ...
        <div className="flex h-8 min-w-0 flex-1 cursor-pointer items-center gap-2 py-0 pl-0.5 text-sm">
          ...
          />
          <InlineRenameInput
            defaultName={t('levelNumbered', { level: level.level })}
            isEditing={isEditing}
            node={level}
            onStartEditing={() => setIsEditing(true)}
            onStopEditing={() => setIsEditing(false)}
        ...
        <Popover onOpenChange={setCameraPopoverOpen} open={cameraPopoverOpen}>
          ...
          >
            <div className="flex flex-col gap-0.5">
              ...
              >
                <Camera className="h-3.5 w-3.5" />
                {level.camera ? t('updateSnapshot') : t('takeSnapshot')}
              </button>
              {level.camera && (
                <button
                  className="flex w-full cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-left text-popover-foreground text-sm hover:bg-destructive hover:text-destructive-foreground"
        ...
        <Popover>
          ...
          <PopoverContent align="start" className="w-40 p-1" side="right">
            <button
              ...
              title={canDeleteLevel ? t('deleteLevel') : t('cannotDeleteGroundLevel')}
            >
              <Trash2 className="h-3.5 w-3.5" />
              {t('delete')}
            </button>
    ...
    <div className="relative flex flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        >
          ...
          <span className="truncate">{t('addLevel')}</span>
        </button>
        {levels.length === 0 && (
  ...
  return (
    <div className="pointer-events-none sticky top-4 z-50 flex h-0 w-full justify-center overflow-visible">
      <div className="pointer-events-auto flex items-center gap-2.5 rounded-full border border-primary/20 bg-primary px-0.5 py-4 pl-2 font-medium text-primary-foreground text-xs shadow-black/10 shadow-lg backdrop-blur-md">
        <span>{t('multiSelect', { count: selectedIds.length })}</span>
        <button
...
function ContentSection() {
  ...
  if (!level) {
    return (
      <div className="px-3 py-4 text-muted-foreground text-sm">{t('selectLevelPrompt')}</div>
    )
  }
  ...
  if (elementChildren.length === 0) {
    return <div className="px-3 py-4 text-muted-foreground text-sm">{t('noElementsOnLevel')}</div>
  }
    ...
    >
      ...
      >
        <div className="flex h-full min-w-0 flex-1 cursor-pointer items-center gap-2 py-2 pl-3">
          ...
          />
          <span className="truncate font-medium text-sm">{building.name || t('defaultBuilding')}</span>
        </div>
        <Popover
        ...
        >
          ...
          >
            <div className="flex flex-col gap-0.5">
              ...
              >
                <Camera className="h-3.5 w-3.5" />
                {building.camera ? t('updateSnapshot') : t('takeSnapshot')}
              </button>
              {building.camera && (
                <button
                  className="flex w-full cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-left text-popover-foreground text-sm hover:bg-destructive hover:text-destructive-foreground"
          ...
          >
            <div className="flex items-center gap-2">
              <img
                ...
                src="/icons/site.png"
              />
              <span className="font-medium text-sm">{siteNode.name || t('defaultSite')}</span>
            </div>
            <CameraPopover
              buttonClassName={cn(
                'transition-colors',
